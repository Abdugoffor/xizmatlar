                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered">
                            <thead>
                                ...
                                <form action="{{ route('bannerphotos.index') }}" method="get">
                                    <tr>
                            ...
                            <td>@if($model->service_photo) <a href="{{ asset($model->service_photo) }}" target="_blank">{{ getTranslation('Open file') }}</a> @else - @endif</td>
                            <td>@if($model->blog_photo) <a href="{{ asset($model->blog_photo) }}" target="_blank">{{ getTranslation('Open file') }}</a> @else - @endif</td>
                            <td>@if($model->team_photo) <a href="{{ asset($model->team_photo) }}" target="_blank">{{ getTranslation('Open file') }}</a> @else - @endif</td>
                            <td>@if($model->contact_photo) <a href="{{ asset($model->contact_photo) }}" target="_blank">{{ getTranslation('Open file') }}</a> @else - @endif</td>

                                        <td>
                                            <div class="d-inline-flex gap-2">
                                                <a href="{{ route('bannerphotos.show', $model->id) }}" class="btn btn-outline-info">
                                                    <i class="icon-eye8"></i>
                                                </a>
                                            ...
                                            <div id="delete_modal_{{ $model->id }}" class="modal fade" tabindex="-1">
                                                <div class="modal-dialog modal-dialog-centered modal-sm">
                                                    <div class="modal-content">
                                                        ...
                                                        <form action="{{ route('bannerphotos.destroy', $model->id) }}" method="post">
                                                            @csrf
                                                            @method('DELETE')
